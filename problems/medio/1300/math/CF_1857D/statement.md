# CF_1857D — Strong Vertices

- Codeforces: [1857D](https://codeforces.com/problemset/problem/1857/D)
- Score/rating: 1300
- Categoria local: `math`
- Tags Codeforces: math, sortings, trees
- Resolvidos no Codeforces: 31370
- Titulo original: D. Strong Vertices
...
Given two arrays `a` and `b`, both of length `n`. Elements of both arrays indexed from `1` to `n`. You are constructing a directed graph, where edge from `u` to `v` (`u!= v`) exists if `a_u-a_v >= b_u-b_v`.
...
The second line of each test case contains `n` integers `a_1,a_2 ... a_n` (`-10^9 <= a_i <= 10^9`) — the array `a`.

The third line of each test case contains `n` integers `b_1,b_2 ... b_n` (`-10^9 <= b_i <= 10^9`) — the array `b`.

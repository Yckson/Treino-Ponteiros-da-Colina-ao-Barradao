...
- Codeforces: [2254D](https://codeforces.com/problemset/problem/2254/D)
...
The second line of each test case contains `n` integers `b_1, b_2, ..., b_n` (`0 <= b_i <= 2 * 10^{14}`) — the shadow array.

It is guaranteed that the sum of `n` over all test cases does not exceed `2 * 10^5`.

## Output

For each test case, output `n` integers `a_1, a_2, ..., a_n` (`1 <= a_i <= 10^{18}`) — the lexicographically smallest valid array `a` that satisfies the condition. If no valid array exists, output `-1` instead.

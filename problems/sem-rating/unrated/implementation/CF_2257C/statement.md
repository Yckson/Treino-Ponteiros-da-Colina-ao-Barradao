# CF_2257C — Spying on the Beaver

- Codeforces: [2257C](https://codeforces.com/contest/2257/problem/C)
- Score/rating: unrated
- Categoria local: `implementation`
- Tags Codeforces: sem tags
- Resolvidos no Codeforces: 0
- Titulo original: C. Spying on the Beaver
...
You are given a rooted tree`^{\text{∗}}` with `n` vertices, numbered from `1` to `n`. The vertex numbered `1` is the root of the tree. The Beaver, initially located at the root, travels through the tree to one of the beaver dams located at the vertices numbered `a_1, ..., a_m`.
...
In the first line of each test case, there is a single integer `n` (`2 <= n <= 10^5`).

The second line contains `n-1` integers `p_2, ..., p_n` — the parents of the vertices from the `2`nd to the `n`th (`1 <= p_i \lt i`; `2 <= i <= n`).

The third line contains a single integer `m` — the number of vertices containing beaver dams (`1 <= m <= n`).

In the fourth line, there are `m` integers `a_1, ..., a_m` — the numbers of these vertices (`1 <= a_i <= n`; `1 <= i <= m`). All `a_i` are distinct.
